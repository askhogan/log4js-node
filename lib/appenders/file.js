  ...
  function openTheStream(file, fileSize, numFiles) {
    ...
    stream.on("error", function (err) {
      ...
      if(err && err.code === 'ENOENT') {
        //keep track of whether creating parent directory has been attempted before.  only
        //allow on first run
        if(createDirectoryFailures.indexOf(file) === -1){
          createDirectoryFailures.push(file);
